...
const createUserDiv = (data) => {
    for(let i = 0; i< data.length; i++){
        ...
        nameItem.textContent = data[i]?.name || '홍길동'
      
        const emailItem = document.createElement('div')
        emailItem.className = 'Item_Info'
        emailItem.textContent = data[i]?.email || 'abc@gmail.com'
      
        const personalItem = document.createElement('div')
        personalItem.className = 'Item_Info'
        personalItem.textContent = data[i]?.personal || '220111-*******'
      
        const phoneItem = document.createElement('div')
        phoneItem.className = 'Item_Info'
        phoneItem.textContent = data[i]?.phone || '010-1234-5678'
      
        const preferItem = document.createElement('div')
        preferItem.className = 'Item_Info'
        preferItem.textContent = data[i]?.prefer || 'https://codebootcamp.co.kr'
